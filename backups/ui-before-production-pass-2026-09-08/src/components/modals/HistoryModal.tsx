                ...
                return (
                  <div key={record.id} className={`history-card ${record.direction} ${isText ? 'text-card' : ''}`}>
                    <div className="history-card-main">
                      ...
                      <div className="history-details" role={isText ? 'button' : undefined} tabIndex={isText ? 0 : undefined} aria-label={isText ? 'อ่านข้อความ ' + record.peerName : undefined} onKeyDown={(event) => { if (isText && (event.key === 'Enter' || event.key === ' ')) { event.preventDefault(); handleTextClick(record); } }} onClick={() => isText && handleTextClick(record)}>
